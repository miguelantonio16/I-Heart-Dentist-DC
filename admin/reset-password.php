...
if (!empty($token)) {
    ...
    $result = $database->query("SELECT * FROM admin WHERE reset_token='$token' AND reset_token_expires > '$current_time'");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !empty($token)) {
    ...
    if ($new_password === $confirm_password) {
        ...
        $email = $admin['aemail'];
        $database->query("UPDATE admin SET apassword='$hashed_password', reset_token=NULL, reset_token_expires=NULL WHERE aemail='$email'");
...
<head>
    ...
    <title>Reset Password - IHeartDentistDC (Admin)</title>
    <style>
        .inside-container {
            height: 700px;
        }
            ...
            <?php if (!empty($token)): ?>
                <form action="" method="POST">
                    ...
                    <div class="password-container">
                        ...
                        <i class="fa-solid fa-eye-slash toggle-icon" id="toggleNewPassword"></i>
                    </div>
