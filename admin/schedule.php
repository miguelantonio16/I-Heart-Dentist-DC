...
// Fetch filtered or all sessions
if ($_POST) {
    $sqlpt1 = "";
    $sqlpt2 = "";
    ...
    $sqlmain = "SELECT schedule.scheduleid, schedule.title, doctor.docname, schedule.scheduledate, schedule.scheduletime, schedule.nop, schedule.status 
                FROM schedule 
                INNER JOIN doctor ON schedule.docid = doctor.docid";

    $conditions = array_filter([$sqlpt1, $sqlpt2]);
    if ($conditions) {
        $sqlmain .= " WHERE " . implode(" AND ", $conditions);
    }
    $sqlmain .= " ORDER BY schedule.status DESC, schedule.scheduledate ASC";
} else {
    $sqlmain = "SELECT schedule.scheduleid, schedule.title, doctor.docname, schedule.scheduledate, schedule.scheduletime, schedule.nop, schedule.status 
                FROM schedule 
                INNER JOIN doctor ON schedule.docid = doctor.docid 
                ORDER BY schedule.status DESC, schedule.scheduledate ASC";
}
...
<head>
    ...
    <title>Sessions - IHeartDentistDC</title>
    <link rel="icon" href="../Media/white-icon/white-IHeartDentistDC_Logo.png" type="image/png">
    <style>
        .popup{
    ...
    <div class="nav-container">
        ...
        <table class="menu-container" border="0">
                            ...
                            <tr>
                                <td colspan="2">
                                    ...
                                    if (isset($_SESSION['temporary_admin']) && $_SESSION['temporary_admin']) {
                                        echo '<a href="switch_back_to_dentist.php"><input type="button" value="Go Back to Dentist View" class="back-btn"></a>';
                                    }
                                    ?>
                                </td>
                            </tr>
                ...
                <tr class="menu-row">
                    <td class="menu-btn menu-icon-doctor">
                        <a href="dentist.php" class="non-style-link-menu">
                            <div>
                                <p class="menu-text">Dentists</p>
                            </div>
                        </a>
                    </td>
                </tr>
        ...
        <div class="dash-body">
            <table border="0" width="100%" style=" border-spacing: 0;margin:0;padding:0;margin-top:25px; ">
                <tr >
                    <td width="13%" >
                    <a href="past_schedule.php" ><button  class="login-btn btn-primary-soft"  style="padding-top:11px;padding-bottom:11px;margin-left:20px;width:125px"><font class="tn-in-text">Past Sessions</font></button></a>
                    </td>
                    <td>
                        <p style="font-size: 23px;padding-left:12px;font-weight: 600;">Session Manager</p>
                    ...
                    <td width="15%">
                        ...
                        $list110 = $database->query("select  * from  schedule where schedule.status='active';");
