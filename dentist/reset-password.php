...
if (!empty($token)) {
    ...
    $result = $database->query("SELECT * FROM doctor WHERE reset_token='$token' AND reset_token_expires > '$current_time'");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !empty($token)) {
    ...
    if ($new_password === $confirm_password) {
        ...
        $database->query("UPDATE doctor SET docpassword='$hashed_password', reset_token=NULL, reset_token_expires=NULL WHERE docemail='$email'");
...
<head>
    ...
    <title>Reset Password - IHeartDentistDC (Dentist)</title>

    <style>
        .password-container .password-toggle-icon {
            position: absolute;
            top: 50%;
            right: 12px;
        ...
        .password-container .password-toggle-icon i {
...
.password-container input[type="text"] {
    width: 100%;
    padding-right: 32px; /* space for the eye icon inside the input */
